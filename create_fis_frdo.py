...
def create_fis_frdo(df:pd.DataFrame,descr_df:pd.DataFrame,folder_template:str,result_folder:str,type_program:str,type_doc:str):
    ...
    try:
        if type_program == 'ДПО':
            ...
            check_name_cols = {'Уровень_образования','Фамилия_в_дипломе','Серия_диплома','Номер_диплома'}
            if check_name_cols.issubset(set(df.columns)):
                # Создаем словарь для хранения номеров колонок для каждого названия
                dct_number_column = {'Номер_удостоверения':7,'Дата_выдачи':8,'Рег_номер':9,'Уровень_образования':15,'Фамилия_в_дипломе':16,'Серия_диплома':17,'Номер_диплома':18,
                                     'Фамилия':22,
                                     'Имя':23,'Отчество':24,
                ...
                dct_number_column = {'Номер_удостоверения': 7, 'Дата_выдачи':8,'Рег_номер': 9,
                                     'Фамилия': 22,
                                     'Имя': 23, 'Отчество': 24,
                                     'Дата_рождения': 25, 'Пол': 26, 'СНИЛС': 27}

                dct_descr_number = {'Тип_программы': 10, 'Наименование_программы': 11,
                                    'Квалификация_профессия_специальность': 14,
                                    'Дата_начало': 19, 'Дата_конец': 20, 'Объём': 21}
